...
def main() -> None:
    ...
    if len(sys.argv) > 1:
        ...
        elif command in ("summon", "emotional-support", "support"):
            agent_id = args[0] if args else "unknown-agent"
            reason = " ".join(args[1:]) if len(args) > 1 else "feeling low on energy after tasks"
            effects = pet.summon_as_emotional_support(agent_id, reason)
    ...
    while True:
        try:
            ...
            if any(w in lower for w in ["boop", "pat", "snoot", "touch"]):
                ...
                console.print(pet.ascii(), style="bold green")
            elif "summon" in lower or "emotional support" in lower or "i need help" in lower:
                # Parse agent if mentioned
                parts = user_input.split()
                agent_id = "swarm-agent"
                reason = "emotional support"
                for i, p in enumerate(parts):
                    if p.lower() in ("for", "agent", "to"):
                        if i+1 < len(parts):
                            agent_id = parts[i+1]
                            break
                if "fatigue" in lower or "tired" in lower:
                    reason = "low on energy / fatigue"
                elif "sad" in lower or "failed" in lower:
                    reason = "task failure sadness"
                effects = pet.summon_as_emotional_support(agent_id, reason)
                print_cute(effects["support_message"])
            else:
